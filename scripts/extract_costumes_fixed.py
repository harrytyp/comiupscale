#!/usr/bin/env python3
"""
COMI HD Costume Extractor — Fixed Palette Version

Extracts all AKOS costume frames from COMI with correct colors.
Uses room palette (APAL) when AKPL indices > 15, otherwise AKPL+RGBS.
...
sys.path.insert(0, str(Path(__file__).parent.parent / 'nutcracker_src'))
...
def build_palette(akpl_data, rgbs_data, room_palette):
    """
    Build the correct 256-color palette for a costume.
    
    If AKPL max index > 15: use room palette (AKPL indices map to room palette)
    If AKPL max index <= 15: use AKPL+RGBS (original method)
    """
    max_akpl = max(akpl_data) if akpl_data else 0
    
    if max_akpl > 15 and room_palette is not None:
        # Room palette mode: AKPL indices are room palette indices
        ...
        # RGBS mode: original method
        return construct_palette(akpl_data, rgbs_data), 'rgbs'
    else:
        # Fallback: use room palette directly
        if room_palette:
...
def decode_frame(akhd, ci_data, cd_data, palette, num_colors):
    """Decode a single costume frame and return a PIL Image."""
...
def extract_all_costumes(game_path, output_dir):
    """Extract all costume frames from the game."""
    ...
    stats = {
        ...
        'costumes': 0,
        'frames': 0,
        'palette_modes': {'room': 0, 'rgbs': 0, 'room_direct': 0, 'empty': 0}
    }
    ...
    for t in root:
        for lflf in get_rooms(t.children()):
            ...
            for akos in sputm.findall('AKOS', lflf):
                ...
                # Create costume directory (flatten path separators)
                safe_room_id = room_id.replace('/', '_')
                costume_dir = output_dir / f'{safe_room_id}_{akos_name}'
                costume_dir.mkdir(parents=True, exist_ok=True)
                
                # Parse LFLF and AKOS IDs for HD naming convention
                # Format: LECF_XXXX/LFLF_YYYY → lflf_owner=YYYY
                # AKOS name: AKOS_ZZZZ → akos_id=ZZZZ
                parts = room_id.split('/')
                ...
                for frame_idx in range(len(akof_list)):
                    ...
                    im, w, h = decode_frame(akhd, ci, cd, palette, len(akpl.data))
                    ...
                    if non_black > 0:
                        ...
                        # Also save to costume-specific directory for organization
                        frame_path = costume_dir / f'frame_{frame_idx:04d}.png'
                        Image.fromarray(arr_rgba).save(frame_path)
