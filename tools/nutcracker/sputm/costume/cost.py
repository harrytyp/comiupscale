    ...
    with io.BytesIO(cost.data) as stream:
        ...
        palette = list(
            itertools.chain.from_iterable(
                [room_palette[3 * x : 3 * x + 3] for x in stream.read(num_colors)],
            ),
        )
        anim_cmds_offset = UINT16LE.unpack(stream.read(UINT16LE.size))[0]
        limbs_offsets = [
            UINT16LE.unpack(stream.read(UINT16LE.size))[0] for _ in range(16)
        ]
        anim_offsets = [
            UINT16LE.unpack(stream.read(UINT16LE.size))[0] for _ in range(num_anim)
        ]
        print(anim_offsets)
        ...
        for t in root:
            for lflf in get_rooms(t.children()):
                ...
                for cost in sputm.findall('COST', lflf):
                    ...
                    for off, im in read_cost_resource(
                        cost,
                        palette,
